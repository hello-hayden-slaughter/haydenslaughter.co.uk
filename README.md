...
`public/Hayden-Slaughter-CV.pdf` and `public/Hayden-Slaughter-Portfolio.pdf` are
**build outputs, never edited here**. They are generated by
`../Portfolio/cv-pdf-build.py`, which copies fresh copies into `public/` on every
run. To update them: make the change in the Portfolio folder (copy master is the
Obsidian vault), run the build, then commit and push this repo. The site links
them at stable root URLs (`/Hayden-Slaughter-CV.pdf`), so shared links always
serve whatever was last deployed.
